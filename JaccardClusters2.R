...
if (length(args)!=3) {
        stop("Wrong parameters. Run as: Rscript JaccardClusters2.R <species-ortholog list> <estimated number of clusters> <list of outlier species>", call.=FALSE)
}
...
#clr2 = gray.colors(100)
jpeg(filename = "jcv_clusters.jpg", height = 5000, width = 5000, units = "px", res = 600)
heatmap(jcvs2, symkey =F, symbreaks=F, scale="none", dendrogram = F, Rowv=F, Colv=F,col = clr, breaks = myBreaks, na.color="white", margin = c(12,16), 
